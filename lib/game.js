    ...
    for (let i = 0; i < this.beams.length; i++) {
      for (let j = 0; j < this.circles.length; j++) {
        ...
        if (this.beams[i].orientation === 'V'
            && this.beams[i].pos[0] > (this.circles[j].pos[0] -
                                                this.circles[j].radius)
            && this.beams[i].pos[0] < this.circles[j].pos[0] +
                                                  this.circles[j].radius
            && (this.beams[i].pos[0] > 1
                && this.beams[i].pos[0] < this.canvas.width * 0.99)) {
              this.enforceCollision(this.circles[j], this.beams[i], i);
        }

        if (this.beams[i].orientation === 'H' &&
            this.beams[i].pos[1] > (this.circles[j].pos[1] -
                                                this.circles[j].radius)
            && this.beams[i].pos[1] < this.circles[j].pos[1] +
                                                  this.circles[j].radius
            && (this.beams[i].pos[1] > 1
                && this.beams[i].pos[1] < this.canvas.height * 0.99)) {
              this.enforceCollision(this.circles[j], this.beams[i], i);
        }
      }
    }
  ...
  enforceCollision(circle, beam, beamIdx) {
    if (circle.color === beam.color) {
      ...
      if (beam.color === "#bf4422") {
        let soundOne = document.getElementsByClassName("hit1")[0];
        soundOne.volume = "0";
        soundOne.currentTime = "0";
        soundOne.volume = "1";
        soundOne.play();
      } else {
        let soundTwo = document.getElementsByClassName("hit2")[0];
        soundTwo.volume = "0";
